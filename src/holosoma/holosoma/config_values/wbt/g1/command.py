...
# NOTE: motion_file below defaults to the bundled largebox motion purely so this experiment is
# runnable/testable out of the box. Override it with your own kicking motion .npz, e.g.:
#   --command.setup_terms.motion_command.params.motion_config.motion_file=/path/to/your/kick.npz
#
# Recovery/hold durations are read from configs/stabilization.yaml at the fork root — edit that
# file to tune them, no code changes needed (see load_stabilization_config's docstring).
_stabilization_cfg = load_stabilization_config()
motion_config_ball_kick = replace(
    motion_config,
    enable_default_pose_append=True,
    default_pose_append_duration_s=_stabilization_cfg.recovery_duration_s,
    post_transition_hold_duration_s=_stabilization_cfg.hold_duration_s,
)

g1_29dof_ball_kick_command = CommandManagerCfg(
    params={},
    setup_terms={
        "motion_command": CommandTermCfg(
            func="holosoma.managers.command.terms.wbt:MotionCommand",
            params={
                "motion_config": motion_config_ball_kick,
            },
        ),
    },
    reset_terms={
