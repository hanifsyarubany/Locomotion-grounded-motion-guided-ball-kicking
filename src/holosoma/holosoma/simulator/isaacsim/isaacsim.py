...
class IsaacSim(BaseSimulator):
    def __init__(self, tyro_config: FullSimConfig, terrain_manager: TerrainManager, device: str):
        ...
        sim_config: SimulationCfg = SimulationCfg(
            ...
            #
            # static_friction/dynamic_friction lowered 1.0 -> 0.8 (2026-07-17): no robot-specific
            # PhysicsConfig.isaacsim override exists anywhere in the G1 config (verified), so this
            # global default is what actually governs the robot's own feet -- including foot<->floor
            # contact, the exact interface an open-loop PD-target replay (identical commands, zero
            # policy, IsaacSim vs MuJoCo) showed diverges within the first control tick during the
            # Stage-C kick's single-support phase (see memory stagec-kick-open-loop-physics-proof).
            # holosoma's own MuJoCo deployment model (RoboJuDo/holosoma's mujoco backend,
            # g1_29dof.xml's explicit <pair .../> foot<->floor contacts) uses friction="0.8 0.8" --
            # PhysX's prior 1.0 was a real, measurable 25% mismatch at exactly that interface
            # (combine_mode="multiply" against terrain's own 1.0 friction -> effective 1.0 vs
            # MuJoCo's explicit 0.8). This does not fully close the gap by itself (an already-wide
            # friction randomization range spanning 0.8 was tried in a prior investigation and did
            # not fix it alone -- see memory stagec-kick-definitive-root-cause-physx-mujoco), but it
            # removes a real, verified, un-randomized default-value mismatch at the nominal center
            # of the training distribution.
            physics_material=sim_utils.RigidBodyMaterialCfg(
                static_friction=0.8,  # was 1.0; matches MuJoCo deploy's explicit foot<->floor pair friction
                dynamic_friction=0.8,  # was 1.0; matches MuJoCo deploy's explicit foot<->floor pair friction
                restitution=0.0,
            ),
        )
    ...
    def _setup_scene(self) -> None:
        ...
        # Populated below, only if a ball is configured -- initialized unconditionally here so
        # get_ball_foot_contact_pos_w() degrades to None via .get(side) rather than AttributeError
        # in a no-ball config.
        self._kick_contact_sensors: dict[str, ContactSensor] = {}
        ...
        # contact_offset/rest_offset (2026-07-17, feet-only rescope attempted+reverted 2026-07-18):
        # previously never configured anywhere in holosoma (see stagec-kick-open-loop-physics-proof
        # / stagec-kick-contactfix-retrain-still-diverges memory for the proven PhysX<->MuJoCo
        # foot-contact divergence this targets). Applied WHOLE-BODY here via collision_props= on
        # the spawn config below.
        #
        # 2026-07-18: attempted rescoping this to feet-only (left/right_ankle_roll_link
        # specifically), motivated by the whole-body version's retrain showing only a transient
        # MuJoCo-survival effect. Blocked by a real USD constraint: G1's URDF-converted asset makes
        # the feet (and ~10 other bodies: knees, hip-rolls, torso, some wrists/shoulders) USD
        # *instances* of read-only shared prototypes, and standard property authoring
        # (modify_collision_properties) unconditionally fails on them
        # (`Tf.ErrorException: ... authoring to an instancing prototype is not allowed`) --
        # confirmed no supported way to disable instancing for URDF-sourced assets in this
        # IsaacLab version (`make_instanceable` is silently not honored by UrdfConverter).
        #
        # Bigger discovery in the process: this WHOLE-BODY call below has the exact same blind
        # spot -- modify_collision_properties's apply_nested wrapper silently SKIPS instanced
        # prims rather than erroring, so this call has ALWAYS silently missed the feet (and knees/
        # hip-rolls/torso/etc), succeeding only on the ~18 non-instanced bodies, even during the
        # retrain+sweep that reported a real transient MuJoCo-survival improvement (best: 7.08/8s
        # at step 201000) in stagec-kick-contact-offset-rest-offset-fix. That result is real
        # MuJoCo-survival data but was never actually testing the ankle/foot-contact interface --
        # whatever caused it came from the other ~18 bodies. Reverted back to this whole-body
        # version anyway (explicit user decision, 2026-07-18) since it's the only config with a
        # measured positive effect so far; the true feet-only fix (editing pre-composition USD
        # source files before Isaac Sim's stage composition makes them read-only) is identified
        # but not yet attempted -- paused pending a decision on priority. Full trail in memory
        # stagec-kick-contact-offset-rest-offset-fix, "MAJOR CORRECTION" section.
        robot_collision_props = None
        if robot_asset_cfg.contact_offset is not None or robot_asset_cfg.rest_offset is not None:
            robot_collision_props = sim_utils.CollisionPropertiesCfg(
                contact_offset=robot_asset_cfg.contact_offset,
                rest_offset=robot_asset_cfg.rest_offset,
            )
        ...
        # add a kickable ball if configured (freely-simulated primitive sphere, not
        # motion-tracked/teleported like the object above — see BallConfig docstring)
        if self.simulator_config.scene.ball is not None:
            ball_cfg_ = self.simulator_config.scene.ball
            ball_name = "ball"  # hardcoded ball name
            ball_cfg = RigidObjectCfg(
                prim_path="/World/envs/env_.*/Ball",
                spawn=sim_utils.SphereCfg(
                    radius=ball_cfg_.radius,
                    rigid_props=sim_utils.RigidBodyPropertiesCfg(
                        disable_gravity=False,
                        retain_accelerations=False,
                        max_linear_velocity=1000.0,
                    ...
                    mass_props=sim_utils.MassPropertiesCfg(mass=ball_cfg_.mass),
                    physics_material=sim_utils.RigidBodyMaterialCfg(
                        static_friction=ball_cfg_.static_friction,
                        dynamic_friction=ball_cfg_.dynamic_friction,
                        restitution=ball_cfg_.restitution,
                    ),
                    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1.0, 1.0, 1.0)),
                    activate_contact_sensors=True,
                ),
                init_state=RigidObjectCfg.InitialStateCfg(
                    pos=ball_cfg_.position,
                ),
            )
            self._ball = RigidObject(ball_cfg)
            self.scene.rigid_objects[ball_name] = self._ball

            # Two narrowly-scoped contact sensors (one per foot), filtered against the ball, for
            # TRUE geometric ball-foot contact detection (managers/reward/terms/shooting.py's
            # has_kicked latch) -- see get_ball_foot_contact_pos_w below. Must stay INSIDE this
            # `scene.ball is not None` block: filter_prim_paths_expr pointing at a Ball prim that
            # will never exist raises inside ContactSensor._initialize_impl() at sim.reset() time.
            # Separate from the broad `contact_sensor` above (which can't be filtered -- IsaacLab
            # only supports filtering when the sensor's own prim_path matches a SINGLE primitive
            # per env, and that one is wildcarded over every robot body). `{ENV_REGEX_NS}` is NOT
            # substituted for imperatively-registered sensors like these (only for scene-cfg-
            # declared entities, which this project doesn't use) -- literal env_.* strings, same
            # convention as `contact_sensor_config`/`ball_cfg` above. Assumes a lowercase
            # left_/right_ foot-body prefix -- true for g1_29dof (the only robot wired to
            # shooting.py today), not necessarily every robot config in this repo; a mismatch fails
            # loudly at sim.reset() (no bodies found), not silently.
            for side in ("left", "right"):
                kick_contact_sensor_config = ContactSensorCfg(
                    prim_path=f"/World/envs/env_.*/Robot/{side}_{self.robot_config.foot_body_name}",
                    filter_prim_paths_expr=["/World/envs/env_.*/Ball"],
                    track_contact_points=True,
                    # update_period/history_length/debug_vis deliberately left at library defaults
                    # (0.0 / 0 / False), NOT mirroring contact_sensor_config above: history_length>0
                    # or debug_vis=True force an eager PhysX recompute on EVERY physics substep
                    # (SensorBase.update()'s force_recompute logic), not just when read. This sensor
                    # is read once per control step for a boolean check, so lazy (recompute only on
                    # .data access) is strictly better here than the broad sensor's own choices,
                    # which serve a different purpose (observations + visualization).
                )
                kick_contact_sensor = ContactSensor(kick_contact_sensor_config)
                self.scene.sensors[f"kick_contact_{side}"] = kick_contact_sensor
                self._kick_contact_sensors[side] = kick_contact_sensor
    ...
    def get_ball_foot_contact_pos_w(self, side: str) -> torch.Tensor | None:
        sensor = self._kick_contact_sensors.get(side)
        if sensor is None:
            return None
        return sensor.data.contact_pos_w[:, 0, 0, :]  # (N, 1, 1, 3) -> (N, 3); NaN where not touching
    ...
    def simulate_at_each_physics_step(self):
        ...
        # Only render if actively recording (not just if video recorder exists). Must check every
        # active recorder (primary + any added via add_video_recorder), not just the primary one:
        # otherwise, whenever the primary recorder happens to be idle between its own recorded
        # intervals while a *different* recorder (e.g. UnifiedManager's dedicated kick-mode
        # recorder) is actively recording, no render pass happens at all this step, so that other
        # recorder's camera annotator returns stale (unchanged) data — every captured frame is a
        # copy of the last real render, producing a frozen/non-updating video.
        has_video_recording = any(r.is_recording for r in self._all_video_recorders())
        ...
        if self.video_recorder or self.extra_video_recorders:
