...
class MuJoCoAllRootStatesProxy:
    """Interleaved multi-actor root-state view for the MuJoCo backend (robot + optional ball).

    Added 2026-07-19 to make Stage-C runnable on this backend. Previously
    ``all_root_states`` was a plain alias of ``robot_root_states``, so the ball was not addressable
    at all and every ball consumer (``kick_ball_pos_b``, the shooting rewards, WBT object pose)
    raised ``NotImplementedError: only supports robot access``.

    ObjectRegistry uses an INTERLEAVED address space:
    ``flat_index = env_id * objects_per_env + offset_in_env``, with the robot at offset 0 and
    individual objects (here: the ball) after it. This proxy resolves those flat indices and routes
    each row to the right underlying per-env view, supporting both reads and writes -- writes matter
    because reset paths do ``sim.all_root_states[indices] = new_states``. A read-only property
    returning a fresh tensor would silently swallow those.

    Deliberately NOT a reimplementation of IsaacSim's proxy/adapter/registry stack (~586 lines): the
    MuJoCo backend has exactly two actor kinds and no scene objects, so a direct two-way router is
    enough. Note the 6 reset call sites that pass ``all_root_states`` WHOLESALE to
    ``set_actor_root_state_tensor`` keep working untouched, because that method identity-checks
    (``root_states is self.all_root_states``) and then uses ``robot_root_states`` directly rather
    than indexing the object it was handed.
    """

    def __init__(self, robot_view, ball_view, objects_per_env: int, device):
        self._robot = robot_view
        self._ball = ball_view
        self._per_env = objects_per_env
        self._device = device
    ...
    def _resolve(self, idx):
        idx = torch.as_tensor(idx, device=self._device, dtype=torch.long).flatten()
        return idx // self._per_env, idx % self._per_env

    def __getitem__(self, key):
        idx, cols = self._split(key)
        env_ids, slots = self._resolve(idx)
        out = torch.zeros(env_ids.numel(), 13, device=self._device, dtype=torch.float32)
        robot_m = slots == 0
        if bool(robot_m.any()):
            out[robot_m] = self._robot[env_ids[robot_m]].to(out.dtype)
        if self._ball is not None:
            ball_m = slots == 1
            if bool(ball_m.any()):
                out[ball_m] = self._ball[env_ids[ball_m]].to(out.dtype)
        return out[:, cols]
    ...
    def __setitem__(self, key, value):
        ...
        if value.shape[0] == 1 and env_ids.numel() > 1:
            value = value.expand(env_ids.numel(), -1)
        for mask, view in ((slots == 0, self._robot), (slots == 1, self._ball)):
            if view is None or not bool(mask.any()):
                continue
            rows = env_ids[mask]
            if isinstance(cols, slice) and cols == slice(None):
                view[rows] = value[mask]
            else:
                cur = view[rows].clone()
                cur[:, cols] = value[mask]
                view[rows] = cur
    ...
    @property
    def shape(self):
        return (self._robot.shape[0] * self._per_env, 13)
...
class MuJoCo(BaseSimulator):
    ...
    def _build_body_index_mapping(self) -> None:
        ...
        # Inverse map as a gather index: holosoma body index -> MuJoCo body id.
        # Needed because the raw per-body state from the backend is [num_envs, model.nbody, ...],
        # i.e. it includes MuJoCo's `world` body at row 0 (and the ball), while `body_names` -- and
        # therefore every consumer, including BaseTask's own `num_bodies = len(body_names)` and
        # MotionCommand's `tracked_body_indexes` -- is ROBOT-ONLY and 0-based. Copying the raw
        # tensor straight across silently shifted every body by one, so motion tracking compared
        # the wrong bodies (measured: |body - root| 5.24m mean / 155m max under mjwarp vs 0.31/0.78
        # under IsaacSim, and Env/motion/error_body_pos 6.94 vs 0.050). See refresh_sim_tensors.
        self._holosoma_body_to_mujoco_id = torch.tensor(
            [
                mujoco.mj_name2id(self.root_model, mujoco.mjtObj.mjOBJ_BODY, self._get_prefixed_name(n))
                for n in self.body_names
            ],
            dtype=torch.long,
            device=self.sim_device,
        )
        assert int((self._holosoma_body_to_mujoco_id < 0).sum()) == 0, (
            f"unmapped body in body_names: {self.body_names}"
        )
    ...
    def load_assets(self):
        ...
        self._set_ball_joint_addressing()
    ...
    def _setup_scene(self) -> None:
        ...
        self.has_ball = self.simulator_config.scene is not None and self.simulator_config.scene.ball is not None

        # Always add robot after terrain, in case it references ground/floor, etc for contacts.
        # add_foot_kick_volume=self.has_ball: only worth the extra collision geoms if there's
        # actually a ball in the scene to kick.
        self.scene_manager.add_robot(
            terrain_state,
            self.robot_config,
            xml_filter=self.simulator_config.robot_mjcf_filter,
            add_foot_kick_volume=self.has_ball,
        )

        # Ball must be added AFTER the robot -- see add_ball()'s docstring for why (robot freejoint
        # must stay the first free joint in the model, robot root must stay body index 1).
        if self.has_ball:
            self.scene_manager.add_ball(self.simulator_config.scene.ball)
            self.ball_radius = self.simulator_config.scene.ball.radius
    ...
    def _set_robot_properties(self) -> None:
        ...
        # Exclude "world" AND the ball: body_names is the ROBOT's body list. Unlike IsaacSim, where
        # the ball is a separate rigid object, here it lives in the same MJCF and would otherwise be
        # appended to this list -- which made the motion loader reject every clip
        # ("The specified name (ball) doesn't exist"), since motion clips only contain robot bodies.
        self.body_names = [name for name in all_body_names if name not in ("world", "ball")]
    ...
    def _set_ball_joint_addressing(self) -> None:
        """Resolve the ball's freejoint qpos/qvel addresses by name (never by scanning/assuming
        position in the model), mirroring `_set_robot_joint_addressing`'s pattern but looked up
        directly via `mj_name2id` since the ball is unprefixed and there's only ever one of it --
        no need for the robot's "first freejoint in the model" scan here."""
        self.ball_qpos_addr: int | None = None
        self.ball_qvel_addr: int | None = None
        if not self.has_ball:
            return

        assert self.root_model
        ball_joint_id = mujoco.mj_name2id(self.root_model, mujoco.mjtObj.mjOBJ_JOINT, "ball_freejoint")
        if ball_joint_id == -1:
            logger.warning("has_ball=True but 'ball_freejoint' not found in compiled model -- disabling ball")
            self.has_ball = False
            return

        self.ball_qpos_addr = int(self.root_model.jnt_qposadr[ball_joint_id])
        self.ball_qvel_addr = int(self.root_model.jnt_dofadr[ball_joint_id])
        logger.info(f"Ball freejoint addressing: qpos_addr={self.ball_qpos_addr}, qvel_addr={self.ball_qvel_addr}")

    def get_ball_position_xy(self) -> tuple[float, float]:
        """Live (x, y) of the ball's actual physics body, read directly from qpos -- reflects
        real contact/kick outcomes, not a canned target. Only valid if `self.has_ball`."""
        assert self.has_ball and self.ball_qpos_addr is not None
        addr = self.ball_qpos_addr
        return float(self.root_data.qpos[addr]), float(self.root_data.qpos[addr + 1])

    def set_ball_position(self, x: float, y: float) -> None:
        """Teleport the ball to (x, y, radius) with identity orientation and zero velocity, then
        run forward kinematics so the viewer/state reflects it immediately. Used for initial spawn
        placement only (see DirectSimulation._arm_ball_broadcast_target) -- once placed, the ball
        is a genuine physics body and moves only through contacts/gravity from then on."""
        assert self.has_ball and self.ball_qpos_addr is not None and self.ball_qvel_addr is not None
        qpos_addr, qvel_addr = self.ball_qpos_addr, self.ball_qvel_addr
        self.root_data.qpos[qpos_addr : qpos_addr + 3] = [x, y, self.ball_radius]
        self.root_data.qpos[qpos_addr + 3 : qpos_addr + 7] = [1.0, 0.0, 0.0, 0.0]
    ...
    def create_envs(self, num_envs, env_origins, base_init_state):
        ...
        # Per-body arrays are addressed by holosoma body index (robot-only), never by MuJoCo model
        # index -- num_bodies is model.nbody and also counts `world` and the ball.
        _n_rb = len(self.body_names) if getattr(self, "body_names", None) else self.num_bodies

        # Initialize contact forces tensor with correct shape [num_envs, n_rb, 3]
        # This matches the interface expected by holosoma (IsaacGym/IsaacSim pattern)
        self.contact_forces = torch.zeros(self.num_envs, _n_rb, 3, device=self.sim_device)

        # Initialize contact forces history tensor to match IsaacGym/IsaacSim pattern
        # Shape: [num_envs, history_length, n_rb, 3]
        history_length = self.simulator_config.contact_sensor_history_length
        self.contact_forces_history = torch.zeros(
            self.num_envs, history_length, _n_rb, 3, device=self.sim_device
        )
    ...
    def prepare_sim(self) -> None:
        ...
        # Setup ObjectRegistry. The ball (when present) is registered as an INDIVIDUAL object so
        # Stage-C consumers can address it -- see MuJoCoAllRootStatesProxy. Scenes still unsupported.
        has_ball_actor = bool(getattr(self, "has_ball", False)) and self.ball_qpos_addr is not None
        self.object_registry.setup_ranges(
            self.num_envs, robot_count=1, scene_count=0, individual_count=1 if has_ball_actor else 0
        )
        ...
        if has_ball_actor:
            ...
            ball_poses[:, 0] = float(self.simulator_config.scene.ball.position[0])
            ball_poses[:, 1] = float(self.simulator_config.scene.ball.position[1])
            ball_poses[:, 2] = float(self.ball_radius)
            ball_poses[:, 6] = 1.0  # xyzw identity
            self.object_registry.register_object("ball", ObjectType.INDIVIDUAL, 0, ball_poses)
        ...
        # all_root_states: interleaved robot(+ball) view. Was previously a bare alias of
        # robot_root_states, which made the ball unaddressable and blocked Stage C on this backend.
        ball_root_view = None
        if has_ball_actor:
            ball_root_view = self.backend.create_root_view(
                {
                    "pos_indices": slice(self.ball_qpos_addr, self.ball_qpos_addr + 3),
                    "quat_indices": slice(self.ball_qpos_addr + 3, self.ball_qpos_addr + 7),
                    "vel_indices": slice(self.ball_qvel_addr, self.ball_qvel_addr + 3),
                    "ang_vel_indices": slice(self.ball_qvel_addr + 3, self.ball_qvel_addr + 6),
                }
            )
        self.all_root_states = MuJoCoAllRootStatesProxy(
            self.robot_root_states,
            ball_root_view,
            self.object_registry.objects_per_env,
            self.sim_device,
        )
        self.ball_root_states = ball_root_view
        ...
        # WarpBackend hands back a zero-copy view of cfrc_ext, which is in MuJoCo MODEL order
        # (world at row 0, plus the ball). Consumers index contact_forces with the SAME holosoma
        # index buffers they use for _rigid_body_* (e.g. env.feet_indices in kick_safety and
        # feet_contact rewards), so it has to be gathered into holosoma order to match. Keep the
        # raw view around and gather into an owned buffer in refresh_sim_tensors().
        _raw_forces = self.backend.create_force_view(self.num_bodies)
        if _raw_forces.shape[1] == len(self.body_names):
            # ClassicBackend already writes holosoma-ordered rows; no reindexing needed.
            self._contact_forces_raw = None
            self.contact_forces = _raw_forces  # type: ignore[assignment]
        else:
            self._contact_forces_raw = _raw_forces
            self.contact_forces = torch.zeros(  # type: ignore[assignment]
                self.num_envs, len(self.body_names), 3, device=self.sim_device
            )
        ...
        # Sized by the ROBOT body count (len(body_names)), NOT model.nbody: nbody also counts
        # MuJoCo's `world` body and the ball, but every consumer indexes these by holosoma body
        # index (BaseTask sets num_bodies = len(body_names)). refresh_sim_tensors gathers into this
        # layout via _holosoma_body_to_mujoco_id.
        _n_rb = len(self.body_names)
        self._rigid_body_pos = torch.zeros(
            self.num_envs, _n_rb, 3, device=self.sim_device, dtype=torch.float32
        )
        self._rigid_body_rot = torch.zeros(
            self.num_envs, _n_rb, 4, device=self.sim_device, dtype=torch.float32
        )
    ...
    def refresh_sim_tensors(self) -> None:
        ...
        if rigid_body_views is not None:
            ...
            # Reindex model-order (includes `world` at row 0, and the ball) -> holosoma body order,
            # which is robot-only and must line up 1:1 with self.body_names. Without this the rows
            # are shifted and motion tracking reads the wrong bodies entirely.
            bidx = self._holosoma_body_to_mujoco_id
            self._rigid_body_pos[:] = positions[:, bidx]
            self._rigid_body_rot[:] = orientations[:, bidx]
            self._rigid_body_vel[:] = linear_vel[:, bidx]
            self._rigid_body_ang_vel[:] = angular_vel[:, bidx]
        else:
            # Slow path: CPU loop with tensor allocation (ClassicBackend)
            assert self.root_model
            assert self.root_data
            for holo_idx, body_id in enumerate(self._holosoma_body_to_mujoco_id.tolist()):
                ...
                self._rigid_body_pos[0, holo_idx] = (
                ...
                self._rigid_body_rot[0, holo_idx] = torch.tensor(
                ...
                self._rigid_body_ang_vel[0, holo_idx] = torch.from_numpy(body_vel[:3]).float().to(self.sim_device)
                self._rigid_body_vel[0, holo_idx] = torch.from_numpy(body_vel[3:]).float().to(self.sim_device)

        # Gather contact forces into holosoma order (see the note where _contact_forces_raw is set).
        if getattr(self, "_contact_forces_raw", None) is not None:
            self.contact_forces[:] = self._contact_forces_raw[:, self._holosoma_body_to_mujoco_id]

        # Update contact force history. The backend writes raw MODEL-order rows, so on the gathered
        # path we roll the history ourselves from the already-reindexed contact_forces instead --
        # otherwise history and contact_forces would disagree about what row N means.
    ...
    def set_actor_states_by_index(self, indices: ActorIndices, states: ActorStates, write_updates: bool = True) -> None:
        ...
        state_offset = 0
        for obj_name, env_ids in resolved_objects:
            if obj_name == "ball" and getattr(self, "ball_root_states", None) is not None:
                # 2026-07-19: ball state setting (the per-reset teleport). Routed through the
                # per-env GPU root view built alongside MuJoCoAllRootStatesProxy rather than the
                # robot branch below -- that branch writes the CPU root_data at HARDCODED qpos[0:3]
                # / qpos[3:7], which is both the robot's freejoint address and single-env only, so
                # it cannot express a per-env ball teleport.
                num_states = len(env_ids)
                obj_states = states[state_offset : state_offset + num_states]
                state_offset += num_states
                idx = torch.as_tensor(env_ids, device=self.sim_device, dtype=torch.long)
                self.ball_root_states[idx] = obj_states.to(self.sim_device, dtype=torch.float32)
                continue
            if obj_name != "robot":
                # TODO: scenes
                raise NotImplementedError(
                    f"MuJoCo simulator supports robot/ball state setting. "
                    f"Object '{obj_name}' is not supported."
                )
    ...
    def get_actor_indices(self, names: str | ActorNames, env_ids: EnvIds | None = None) -> ActorIndices:
        ...
        for name in names:
            # "ball" became addressable 2026-07-19 (registered as an INDIVIDUAL object + the
            # interleaved MuJoCoAllRootStatesProxy). Anything else is still unsupported; the
            # registry would raise a bare KeyError, so keep an explanatory error here.
            if name not in self.object_registry.name_to_index:
                raise NotImplementedError(
                    f"MuJoCo simulator supports {sorted(self.object_registry.name_to_index)} access. "
                    f"Requested object '{name}' is not registered (scenes are still unimplemented)."
                )
    ...
    def find_rigid_body_indice(self, body_name: str) -> int:
        ...
        """
        # Must return the HOLOSOMA index (position in self.body_names), NOT the MuJoCo model body
        # id. Every consumer -- feet_indices, feet_height_indices, termination_contact_indices,
        # torso_index, kick_foot_index -- uses the result to index per-body arrays
        # (_rigid_body_pos/rot/vel/ang_vel, contact_forces), and all of those are in holosoma
        # robot-only order. IsaacSim.find_rigid_body_indice has the same contract.
        #
        # This previously returned mj_name2id(...), i.e. MODEL order, which counts MuJoCo's `world`
        # body at row 0 and the ball. That was accidentally consistent while _rigid_body_* was also
        # stored in model order, but the 2026-07-20 body-index fix moved those arrays to holosoma
        # order and left this function behind -- shifting every lookup by one, so feet_phase read
        # the hip and the waist instead of the two feet and saturated to exactly 0.0.
        # Use find_mujoco_body_id() when a genuine MuJoCo model id is needed (e.g. xfrc_applied).
        try:
            return self.body_names.index(body_name)
        except ValueError:
            raise RuntimeError(f"Body '{body_name}' not found in body_names: {self.body_names}") from None

    def find_mujoco_body_id(self, body_name: str) -> int:
        """Find the MuJoCo *model* body id for a body name.

        This is the index into MuJoCo model/data arrays (xpos, xquat, cfrc_ext, xfrc_applied), which
        includes the `world` body at row 0 and any non-robot bodies such as the ball. It is NOT
        interchangeable with find_rigid_body_indice() -- use this one only when addressing MuJoCo
        structures directly.
        ...
        raise RuntimeError(f"Body '{body_name}' not found in MuJoCo model (prefixed: {prefixed_name!r})")
