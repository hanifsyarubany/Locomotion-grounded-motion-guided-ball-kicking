...
class BaseSimulator:
    ...
    **Batched Updates (Performance)**:
    ...
    def __init__(self, tyro_config: FullSimConfig, terrain_manager: TerrainManager, device: str):
        ...
        # Additional simultaneous recorders beyond the primary one above — e.g. UnifiedManager
        # adds one here to record a second, dedicated env to a second wandb media key (see
        # add_video_recorder). Empty for every other env class, so this is a no-op for them.
        self.extra_video_recorders: list[VideoRecorderInterface] = []
    ...
    def get_ball_foot_contact_pos_w(self, side: str) -> torch.Tensor | None:
        """[num_envs, 3] world-frame ball-foot contact position for side ('left'/'right'), NaN in
        envs where that foot isn't currently touching the ball. IsaacSim only (PhysX ContactSensor
        with filter_prim_paths_expr) -- returns None on backends without this (MuJoCo, IsaacGym);
        callers must fall back to a geometric approximation there (see shooting.py's
        _is_foot_contact)."""
        return None
    ...
    def _all_video_recorders(self) -> list[VideoRecorderInterface]:
        """The primary recorder (if any) plus any recorders added via add_video_recorder, in
        registration order."""
        recorders = [] if self.video_recorder is None else [self.video_recorder]
        recorders.extend(self.extra_video_recorders)
        return recorders

    def add_video_recorder(self, recorder: VideoRecorderInterface) -> None:
        """Register an additional, simultaneously-active video recorder — e.g. to record a second
        env to a second wandb media key (give it a distinct VideoConfig.wandb_key). Calls
        recorder.setup_recording() immediately, so only call this once the scene is fully built
        (safe any time after simulator construction returns, e.g. from an env class's
        _init_buffers())."""
    ...
    def on_episode_start(self, env_id: int = 0) -> None:
        ...
        for recorder in self._all_video_recorders():
    ...
    def capture_video_frame(self) -> None:
        """Capture a video frame during simulation, for every active recorder.

        This method should be called once per simulation step (not per env — the sim steps all
        envs in lockstep). Each recorder is only ever bound to its own configured
        VideoConfig.record_env_id, so this dispatches to each recorder with that recorder's own
        env id rather than a single shared one.
        """
        for recorder in self._all_video_recorders():
            recorder.capture_frame(recorder.config.record_env_id)
