...
class LoggingHelper:
    ...
    def post_epoch_logging(
        ...
        # Opt-out-of-"Env/" convention: any env log_dict key of the form "Section::metric" gets
        # pulled into its OWN top-level wandb/tensorboard section (sibling to "Env", "Loss", etc,
        # via the existing extra_log_dicts mechanism below) instead of being nested under "Env/".
        # Generic on purpose -- this file has no idea what "kick" or "skill" means; any env class's
        # log_dict producer can opt a metric into its own section this way. Plain keys (no "::")
        # keep today's exact behavior, unaffected. First user: UnifiedManager's per-skill kick
        # breakdown (e.g. "Kick_skills_0::kick_alive_frac" -> wandb key "Kick_skills_0/kick_alive_frac").
        sectioned_from_env: dict[str, dict[str, float]] = {}
        plain_env_log_dict: dict[str, float] = {}
        for k, v in env_log_dict.items():
            if "::" in k:
                section, _, metric = k.partition("::")
                sectioned_from_env.setdefault(section, {})[metric] = v
            else:
                plain_env_log_dict[k] = v
        env_log_dict = {f"Env/{k}": v for k, v in plain_env_log_dict.items()}
        if sectioned_from_env:
            extra_log_dicts = {k: dict(v) for k, v in extra_log_dicts.items()}  # don't mutate caller's dict
            for section, metrics in sectioned_from_env.items():
                extra_log_dicts.setdefault(section, {}).update(metrics)
