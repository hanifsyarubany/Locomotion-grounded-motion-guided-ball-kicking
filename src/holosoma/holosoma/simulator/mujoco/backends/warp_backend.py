...
class WarpBackend(IMujocoBackend):
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: FullSimConfig, device: str):
        ...
        with wp.ScopedDevice(self.mjw_device):
            ...
            # 2026-07-18: put_model() unconditionally hardcodes opt.tolerance to 1e-6 internally
            # (confirmed empirically -- setting it on the CPU model beforehand has no effect,
            # since put_model() overrides it regardless), 100x looser than MuJoCo's own default
            # (1e-8, confirmed via mujoco.MjOption().tolerance -- the same value RoboJuDo's
            # proven-stable Classic deployment scene actually uses). Empirically implicated in a
            # real instability: real Stage-B training under simulator:mjwarp (num_envs>=64)
            # reliably diverges to NaN within ~100-150 iterations; a pure-rollout diagnostic
            # probe (no gradient updates) reproduced a 2000+N contact-force spike at step 0 using
            # STOCHASTIC actions (matching real training's actor.explore() rollout collection) --
            # a purely deterministic-action probe over 2000 steps never showed this.
            # iterations/ls_iterations were confirmed to already match the CPU model (100/50) --
            # tolerance was the one solver-config discrepancy found. Tighten it back to MuJoCo's
            # own default post-upload (this field is a wp.array, can't be set pre-upload) so the
            # GPU solver's convergence criterion matches the reference (CPU) solver exactly, same
            # "match the known-good default" philosophy as the earlier friction/contact_offset
            # fixes applied to the isaacsim backend for the same PhysX<->MuJoCo divergence
            # investigation. NOT yet proven to close the gap -- only re-running the same
            # diagnostic probe (and eventually a real training run) can prove that.
            self.mjw_model.opt.tolerance = wp.array(
                [mujoco.MjOption().tolerance],
                dtype=self.mjw_model.opt.tolerance.dtype,
                device=self.mjw_model.opt.tolerance.device,
            )
        ...
        # 2026-07-18: expand the per-world model fields BEFORE capturing the step graph below, so
        # the graph (and the collision kernels that read m.geom_friction / m.body_mass etc. per
        # world) reference the SAME per-world arrays the runtime writers update. Ported from
        # upstream holosoma (amazon-far/holosoma PR #147, "backend-agnostic scene objects +
        # link_physics", merged 2026-07-09) -- this fork's warp_backend.py predates that PR and
        # went straight from tensor creation to graph capture with no expansion at all, while
        # `expand_model_fields` (fields.py/warp_randomization.py) already existed here and was
        # already documented "must be called BEFORE the randomization manager is initialized" --
        # it just wasn't actually being called that early. `expand_model_fields` reallocates these
        # arrays; doing it after capture (the previous behavior here) leaves the captured graph
        # referencing the ORIGINAL (single-world, since-reallocated) arrays -- not just "DR writes
        # never affect dynamics" as upstream's own PR description states, but genuinely reading
        # stale/dangling GPU memory once the later prepare_fields()/prepare_manager_fields() call
        # (fields.py) reallocates -- a real memory-safety bug, and the leading suspect for the
        # NaN-divergence investigation in memory stagec-kick-... / mjwarp-training-instability-...
        # (corruption spanning multiple unrelated observation terms in one contiguous block, i.e.
        # exactly what stale/reallocated shared GPU memory would produce). Idempotent, so the
        # later prepare_fields()/in-place writes never reallocate again.
        #
        # The list = body_pos/body_quat (set_static_body_world_pose) PLUS every field a
        # @mujoco_required_field randomization term writes (geom_friction/geom_solref/geom_solimp/
        # body_mass/body_inertia/body_ipos/dof_damping) PLUS cam_pos (camera sensors, not yet used
        # on this fork but harmless to include and keeps this list matched to upstream's).
        if self.num_envs > 1:
            from .warp_randomization import expand_model_fields
