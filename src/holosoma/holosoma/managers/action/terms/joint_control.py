...
class JointPositionActionTerm(ActionTermBase):
    ...
    def __init__(self, cfg: ActionTermCfg, env: Any):
        ...
        # 2026-08-05, ported from RoboNaldo -- see RobotControlConfig.per_joint_action_clip's own
        # docstring for the full derivation/rationale. None or {} (default): both bounds tensors
        # are exactly (-action_clip_value, action_clip_value) for every joint, so process_actions'
        # per-joint torch.clip below is byte-identical to the pre-existing scalar clip.
        self._action_clip_low = torch.full(
            (self._action_dim,), -env.robot_config.control.action_clip_value, device=env.device
        )
        self._action_clip_high = torch.full(
            (self._action_dim,), env.robot_config.control.action_clip_value, device=env.device
        )
        self._configure_per_joint_action_clip(env)
    ...
    def process_actions(self, actions: torch.Tensor) -> None:
        ...
        # Clip actions. self._action_clip_low/_action_clip_high are per-joint bounds, resolved
        # once at construction (_configure_per_joint_action_clip) -- (-action_clip_value,
        # action_clip_value) for every joint unless RobotControlConfig.per_joint_action_clip
        # overrides specific joint types, so this is byte-identical to the pre-2026-08-05 scalar
        # clip whenever that field is unset.
        if self.env.robot_config.control.clip_actions:
            assert self._processed_actions is not None
            self._processed_actions[:] = torch.clip(actions, self._action_clip_low, self._action_clip_high)
            # Log clipping fraction
            self.env.log_dict["action_clip_frac"] = (
                (self._processed_actions == self._action_clip_low) | (self._processed_actions == self._action_clip_high)
    ...
    def _configure_per_joint_action_clip(self, env: Any) -> None:
        """Resolve RobotControlConfig.per_joint_action_clip (2026-08-05, ported from RoboNaldo)
        into self._action_clip_low/_action_clip_high -- see that field's own docstring. Mirrors
        _configure_pd_gains's substring-matching convention exactly (a joint TYPE key like
        "ankle_roll" matches every DOF name containing it)."""
        per_joint_clip = getattr(env.robot_config.control, "per_joint_action_clip", None)
        if not per_joint_clip:
            return
        for i, name in enumerate(env.dof_names):
            for joint_type, (low, high) in per_joint_clip.items():
                if joint_type in name:
                    self._action_clip_low[i] = low
                    self._action_clip_high[i] = high
