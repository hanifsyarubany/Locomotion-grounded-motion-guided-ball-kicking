...
from holosoma.config_types.simulator import BallConfig, MujocoXMLFilterCfg, SimulatorConfig
...
class MujocoSceneManager:
    ...
    def add_robot(
        ...
        prefix: str = "robot_",
        add_foot_kick_volume: bool = False,
    ) -> None:
        ...
        add_foot_kick_volume : bool
            If True, add a supplementary box collision geom to each foot (see
            `_add_foot_kick_volume`'s docstring) -- pass True whenever a ball is also being added
            via `add_ball`, since the MJCF's native foot collision geoms are too thin to reliably
            contact a ball during a kick swing.
        """
        ...
        if add_foot_kick_volume:
            self._add_foot_kick_volume(robot_spec)
    ...
    def _add_foot_kick_volume(self, robot_spec: mujoco.MjSpec) -> None:
        """Add a supplementary box collision geom to each foot, spanning its real footprint from
        sole through ankle height (x in [-0.054, 0.132], per the native foot collision geoms'
        own fromto extents).

        The MJCF's native foot collision geoms (`{side}_foot{1-7}_collision`) are thin (1cm-radius)
        capsules positioned purely at the sole (z=-0.025 relative to the ankle) -- built only for
        foot-floor walking contact, never sized to reliably contact an 11cm-radius kickable ball
        during a kick swing. This exact fix (ported from
        `high_level_ball_approach_and_kicking/src/hl_kick/eval_hl_mujoco.py`'s standalone MuJoCo
        eval, where it roughly doubled kick success with zero regression on the fall rate -- see
        that project's README, "Root cause found" section) adds a box spanning that same footprint
        from the sole up through ankle height.

        Deliberately placed on its own collision channel (contype=conaffinity=4) rather than
        reusing the existing floor-contact channel: the first attempt (in the sibling project)
        shared the floor's channel and made things *worse* (new double-contact against the floor
        during ordinary walking), since this box's z-range overlaps the sole capsules' own
        ground-contact zone. Isolating it onto channel 4 means it only ever contacts geoms that
        explicitly opt into channel 4 too -- i.e. the ball added via `add_ball` (conaffinity
        includes bit 4), never the floor or the robot's own other geoms.
        """
        for side in ("left", "right"):
            body = next((b for b in robot_spec.bodies if b.name == f"{side}_ankle_roll_link"), None)
            if body is None:
                logger.warning(f"add_foot_kick_volume: body '{side}_ankle_roll_link' not found, skipping")
                continue
            x_lo, x_hi = -0.054, 0.132
            body.add_geom(
                name=f"{side}_foot_kick_collision",
                type=mujoco.mjtGeom.mjGEOM_BOX,
                pos=[(x_lo + x_hi) / 2, 0.0, -0.0025],
                size=[(x_hi - x_lo) / 2, 0.026, 0.0225],
                contype=4,
                conaffinity=4,
                group=3,
                rgba=[0.1, 0.6, 0.9, 0.3],
            )
        logger.info("Added foot-kick collision volumes (left/right, channel 4)")

    def add_ball(self, ball_cfg: BallConfig) -> None:
        """Add a freely-simulated, visible, kickable ball directly to the world spec.

        Must be called AFTER `add_robot()` -- two things elsewhere in this MuJoCo backend rely on
        the robot's freejoint/root body staying first in the compiled model:
        `MuJoCo._set_robot_joint_addressing()` finds the robot's freejoint by scanning for the
        *first* free joint in the whole model, and a couple of spots (e.g. camera tracking in
        `MuJoCo.render()`) hardcode the robot's root body as body index 1 (right after the world
        body). Adding the ball's body/freejoint after the robot is attached keeps both assumptions
        correct -- the ball's freejoint and body simply come after the robot's in the compiled
        model's ordering.

        The ball is unprefixed (unlike the robot, which is namespaced under `"robot_"` by
        `add_robot`'s `attach()`) -- its qpos/qvel are resolved by plain name lookup
        (`"ball_freejoint"`) in `MuJoCo._set_ball_joint_addressing()`.

        Collision channels: `contype=1` (shares the robot's own geom contype, so it matches
        whichever `conaffinity` `_configure_robot_collisions` assigned -- 2 for environment-only,
        3 for self+environment); `conaffinity=7` (bits 1+2+4) -- bit 1 covers the floor (terrain's
        `conaffinity=1`), bit 2 is redundant coverage for the same floor collision via the other
        matching rule, and bit 4 covers the foot-kick-volume boxes from `_add_foot_kick_volume`
        (channel 4) if present. Net effect: the ball collides with the floor, the robot's normal
        collision geoms, and the foot-kick-volume boxes, regardless of the self-collision setting.
        """
        ball = self.world_spec.worldbody.add_body(name="ball", pos=list(ball_cfg.position))
        ball.add_freejoint(name="ball_freejoint")
        ball.add_geom(
            name="ball_geom",
            type=mujoco.mjtGeom.mjGEOM_SPHERE,
            size=[ball_cfg.radius, 0, 0],
            mass=ball_cfg.mass,
            friction=[ball_cfg.dynamic_friction, 0.005, 0.0001],
            condim=4,
            contype=1,
            conaffinity=7,
            rgba=[0.9, 0.25, 0.1, 1.0],
        )
        logger.info(f"Added ball: radius={ball_cfg.radius}m, mass={ball_cfg.mass}kg, init pos={ball_cfg.position}")
