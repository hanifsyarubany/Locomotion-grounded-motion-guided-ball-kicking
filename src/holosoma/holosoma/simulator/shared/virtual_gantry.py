...
class VirtualGantry:
    ...
    def _apply_force_mujoco(self, link_id: int, force: npt.NDArray[np.float64]) -> None:
        ...
        ----------
        link_id : int
            Holosoma body index of the rigid body to apply force to.
        force : npt.NDArray[np.float64]
            3D force vector [fx, fy, fz] to apply.
        """
        env_id = 0  # Virtual gantry only supports single environment

        # applied_forces is MuJoCo's xfrc_applied, addressed by MODEL body id, but body_link_id is a
        # holosoma index (that is what find_rigid_body_indice returns, and what the IsaacSim path
        # below feeds into sim.body_ids). Translate here rather than storing a second convention.
        mj_id = self._to_mujoco_body_id(link_id)
        ...
        if isinstance(self.sim.applied_forces, torch.Tensor):
            ...
            self.sim.applied_forces[env_id, mj_id, :3] = force_tensor
        else:
            # ClassicBackend: CPU numpy array without env dimension [num_bodies, 6]
            self.sim.applied_forces[mj_id, :3] = force

    def _to_mujoco_body_id(self, holosoma_idx: int) -> int:
        """Map a holosoma body index to the MuJoCo model body id used by xfrc_applied."""
        mapping = getattr(self.sim, "_holosoma_body_to_mujoco_id", None)
        if mapping is None:
            return holosoma_idx
        return int(mapping[holosoma_idx])
    ...
    def _clear_forces_mujoco(self) -> None:
        ...
        if isinstance(self.sim.applied_forces, torch.Tensor):
            ...
            # Same holosoma -> MuJoCo model id translation as _apply_force_mujoco.
            self.sim.applied_forces[env_id, self._to_mujoco_body_id(self.body_link_id), :] = 0.0
