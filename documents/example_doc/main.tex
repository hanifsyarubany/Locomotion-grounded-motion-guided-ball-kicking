...
\toprule
& & \multicolumn{4}{c}{\textbf{Locomotion substrate}} & \multicolumn{4}{c}{\textbf{Skill repertoire \& transitions}} \\
\cmidrule(lr){3-6}\cmidrule(lr){7-10}
Method & Platform & Free & Omni & Terr. & Loco. & Multi & Weak & Pass & Loco.\ \\
 & & cmd. & dir. & & eval. & skill & foot & & handoff \\
\midrule
Haarnoja et al.~\citep{haarnoja2024soccer} & Miniature & \yes & \partialmark & \no & \no & \yes & \no & \no & \yes \\
...
\midrule
\textbf{Ours (proposed)}       & Humanoid & \yes & \yes & \yes & \yes & \yes & \yes & \yes & \yes \\
\bottomrule
\end{tabular}
...
\emph{Terr.}: non-flat terrain.
\emph{Loco.\ eval.}: locomotion metrics (command tracking, push recovery) actually reported.
\emph{Multi skill}: more than one distinct strike/contact type.
\emph{Weak foot}: strikes with both limbs.
\emph{Pass}: weighted pass as a distinct contact modality, not a nearer shooting target.
\emph{Loco.\ handoff}: post-strike control returns to a trained locomotion controller rather than a scripted stabilisation reference.
Marks reflect capabilities \emph{as reported in each paper}; an unreported capability is scored \no\ or \partialmark\ rather than assumed absent. The \partialmark\ in RoboNaldo's \emph{Free cmd.}\ column reflects a locomotion-command interface whose commands originate from a ball-approach heuristic, with tracking rewards active throughout; its \partialmark\ under \emph{Loco.\ handoff} reflects a scripted per-clip stabilisation reference rather than a return to locomotion control.}
