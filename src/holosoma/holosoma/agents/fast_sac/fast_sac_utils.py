...
class SimpleReplayBuffer(nn.Module):
    def __init__(
        ...
        sanitize_enabled: see FastSACConfig.replay_buffer_sanitize_enabled's own docstring --
        False (default) is an exact no-op, byte-identical to before this parameter existed.
        """
        super().__init__()
        ...
        # 2026-07-28: which SAC entropy group (0=locomotion, 1=kick) each transition belongs to --
        # see FastSACConfig.kick_target_entropy_ratio's docstring. Always present (mirrors
        # dones/truncations' pattern exactly) even for envs with no task_mode concept at all
        # (locomotion-only/WBT-only experiments) -- extend() fills it with all-zeros for those via
        # a safe getattr fallback, and it's simply never consulted when kick_target_entropy_ratio
        # is unset (the default), so this is inert for every experiment that doesn't opt in.
        self.is_kick = torch.zeros((n_env, buffer_size), device=device, dtype=torch.long)
        # 2026-08-15: which motion SKILL (0..N-1) each transition belongs to -- exact sibling of
        # is_kick above (same shape, same extend/gather/fallback discipline), added for
        # FastSACConfig.skill_replay_weights. Distinct from is_kick, which is only the 2-value
        # locomotion-vs-kick label; this is the N-value per-skill label. Meaningless (but harmless,
        # and never consulted) for locomotion transitions -- the weighting path masks on is_kick.
        # All-zeros for envs with no skill concept at all, via extend()'s getattr fallback.
        self.skill_id = torch.zeros((n_env, buffer_size), device=device, dtype=torch.long)
    ...
    def extend(
        ...
        is_kick = tensor_dict["is_kick"]
        # OPTIONAL, unlike is_kick above: a caller with no skill concept at all (locomotion-only /
        # WBT-only experiments, and this class used standalone) may omit it, and all-zeros is the
        # semantically correct default there -- "no skill information" == the single skill 0. The
        # real training path (FastSACAgent.learn) always supplies it explicitly.
        skill_id = tensor_dict["skill_id"] if "skill_id" in tensor_dict.keys() else torch.zeros_like(is_kick)
        ...
        if self.sanitize_enabled:
            # See FastSACConfig.replay_buffer_sanitize_enabled's own docstring: a last-line-of-
            # defense guard against a rare per-env physics-solver numerical explosion reaching the
            # learner as a NaN/Inf transition. actions/dones/truncations/is_kick are deliberately
            # NOT touched here -- actions come from the (already-clipped) policy, and
            # dones/truncations/is_kick are integer/bool flags with no continuous-value blowup
            # mode; only the value-bearing float tensors sourced from the environment's own
            # reward/observation computation are at risk.
            observations = torch.nan_to_num(observations)
            rewards = torch.nan_to_num(rewards)
            next_observations = torch.nan_to_num(next_observations)
            critic_observations = torch.nan_to_num(critic_observations)
            next_critic_observations = torch.nan_to_num(next_critic_observations)
        ...
        self.is_kick[:, ptr] = is_kick
        self.skill_id[:, ptr] = skill_id
    ...
    def sample(self, batch_size: int):
        ...
        if self.n_steps == 1:
            ...
            is_kick = torch.gather(self.is_kick, 1, indices).reshape(self.n_env * batch_size)
            skill_id = torch.gather(self.skill_id, 1, indices).reshape(self.n_env * batch_size)
            ...
            observations = torch.gather(self.observations, 1, obs_indices).reshape(self.n_env * batch_size, self.n_obs)
            actions = torch.gather(self.actions, 1, act_indices).reshape(self.n_env * batch_size, self.n_act)
            # is_kick describes THIS transition's own task assignment (env-permanent, never
            # changes within/across episodes) -- gathered via the BASE index like
            # observations/actions above, not the n-step-lookahead final_next_obs_indices used for
            # next_observations/dones/truncations below.
            is_kick = torch.gather(self.is_kick, 1, indices).reshape(self.n_env * batch_size)
            skill_id = torch.gather(self.skill_id, 1, indices).reshape(self.n_env * batch_size)
            # Gather full critic observations
            critic_obs_indices = indices.unsqueeze(-1).expand(-1, -1, self.n_critic_obs)
