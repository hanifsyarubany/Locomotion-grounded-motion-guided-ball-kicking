...
class TerrainLocomotion(TerrainTermBase):
    ...
    def _get_env_origins(self):
        ...
        if self._cfg.spawn.randomize_tiles:
            ...
            origins = self.terrain.sample_env_origins()
            self._env_origins[:] = torch.from_numpy(origins).to(self.device).to(torch.float)
            levels, types = self.terrain._last_terrain_levels, self.terrain._last_terrain_types
        else:
            ...
            levels = np.zeros(self.num_envs, dtype=np.int64)
            types = np.zeros(self.num_envs, dtype=np.int64)
    ...
    def env_terrain_is_flat(self) -> torch.Tensor:
        """Bool tensor [num_envs]: whether each env's (static, never-resampled) terrain tile is
        flat. Populated for every mesh type — uniformly True for "plane" terrain, and correctly
        selective for the "trimesh" mixed-terrain generator; all-False for mesh types that never
        expose per-cell terrain names (load_obj, none), which is the safe default (never assume
        flatness where it hasn't been verified)."""
        return self._env_terrain_is_flat
