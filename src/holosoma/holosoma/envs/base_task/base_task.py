...
class BaseTask:
    def __init__(
        ...
        *,
        ...
        # 2026-08-15: hook for subclass state that must exist before ANY manager is constructed
        # below -- see its own docstring for why this exists (RewardManager/TerminationManager's
        # per-skill weight/param validation needs env.skill_id at THEIR construction time, which
        # is well before _init_buffers() near the bottom of this method, where skill_id is
        # normally (re)computed once command_manager also exists). No-op by default.
        self._pre_manager_construction_callback()
    ...
    def _pre_manager_construction_callback(self):
        """Hook run immediately after num_envs/device are set, BEFORE terrain_manager/simulator/
        any of the reward/termination/observation/action/command/etc. managers are constructed --
        the earliest point in __init__ at which per-env [num_envs]-shaped subclass state can
        exist. No-op by default.

        Added 2026-08-15 for UnifiedManager's skill_id specifically: RewardManager/
        TerminationManager (managers/reward/manager.py, managers/termination/manager.py) validate
        at THEIR OWN construction time that env.skill_id already exists whenever any term has a
        per-skill weight/param table (see "simultaneous per-skill task configs") -- but
        UnifiedManager's own skill_id is normally (re)computed in _init_buffers(), which needs
        command_manager (constructed AFTER reward_manager/termination_manager, near the bottom of
        this method) and so runs far too late for that validation to see it. Subclasses needing
        state this early should override this rather than relying on _init_buffers()."""
        return
