...
class CommandManager:
    ...
    def _filter_env_ids_for_task_mode(self, env_ids, task_mode: str | None):
        """Restrict env_ids to those currently in the given task mode.

        No-op (returns env_ids unchanged) unless both the term opts in (``task_mode is not
        None``) and the env implements ``task_mode_mask`` (e.g. UnifiedManager) — so this is
        inert for every existing experiment.
        """
        if task_mode is None or not hasattr(self.env, "task_mode_mask"):
            return env_ids
        mask = self.env.task_mode_mask(task_mode)
        return env_ids[mask[env_ids]]

    def reset(self, env_ids) -> None:
        reset_cfg_by_name = dict(zip(self._reset_names, self._reset_cfgs))

        for entry in self._class_entries:
            if "reset" in entry["stages"]:
                term_cfg = reset_cfg_by_name.get(entry["name"])
                task_mode = term_cfg.task_mode if term_cfg is not None else None
                entry["instance"].reset(self._filter_env_ids_for_task_mode(env_ids, task_mode))
        ...
        for term_name, term_cfg in zip(self._reset_names, self._reset_cfgs):
            if term_name in self._reset_funcs:
                ...
                func(self.env, self._filter_env_ids_for_task_mode(env_ids, term_cfg.task_mode), **term_cfg.params)
