...
def _select_policy_class(config: InferenceConfig):
    ...
    # Unified locomotion+kick: a single policy with a merged, prefixed observation
    # space (loco_*/kick_* to avoid name collisions between the two source term
    # sets), tagged by this always-present, un-prefixed marker term. Checked before
    # the WBT heuristic below since "motion_command" (exact match) never matches our
    # prefixed "kick_motion_command".
    if "task_mode_onehot" in actor_obs:
