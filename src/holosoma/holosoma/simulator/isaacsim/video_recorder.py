    ...
    def _update_camera_position(self) -> None:
        ...
        try:
            ...
            self._view.set_world_poses(
                position,
                orientations,
                # Index into this *view's own* managed-prim list, not the absolute env id: self._view
                # was constructed from a single literal prim path (this recorder's one camera), so it
                # always manages exactly one prim at index 0 — regardless of which env's camera that
                # happens to be. Passing record_env_id here only worked by coincidence when it was 0
                # (the default); for any other record_env_id it indexes past the view's single entry
                # and set_world_poses silently no-ops (camera position never updates).
