...
# Must run -- and its own import must happen -- before any holosoma.config_types.env /
# .experiment / config_values import below: those transitively import
# config_values/unified/g1/reward.py, which calls load_ball_config() at MODULE IMPORT time (see
# that file's comment). config_types.simulator itself imports no config_values module (verified),
# so it's safe to import first and apply CLI ball-x/y overrides before anything downstream reads
# them.
...
def train(tyro_config: ExperimentConfig, training_context: TrainingContext | None = None) -> None:
    ...
    try:
        ...
        if wandb_enabled and is_main_process:
            ...
            # dataclasses.asdict already bakes the resolved ball position/target into
            # simulator.config.scene.ball (load_ball_config() ran at import time, see
            # config_types/simulator.py) -- so a --ball.x/--ball.y override IS correctly reflected
            # in this config, just nested there rather than under a literal "ball.x" key. Surface
            # it a second time under an explicit, easy-to-find key too: --ball.x/--ball.y never
            # reach wandb's own auto-captured command/args (they're stripped from sys.argv by
            # apply_ball_cli_overrides before wandb.init ever runs, since they have to be gone
            # before tyro sees them -- see that function's docstring), so without this a run
            # launched with a manual ball override would look, from wandb's Overview tab alone,
            # identical to one that wasn't -- only this dict and scene.ball.position would differ.
