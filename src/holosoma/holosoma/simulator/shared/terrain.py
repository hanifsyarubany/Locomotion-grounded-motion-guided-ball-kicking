...
class Terrain(TerrainInterface):
    ...
    def _initialize_terrain_config(self) -> trimesh.Trimesh:
        ...
        # Per-cell terrain-type name, populated by randomized_terrain() below. Lets callers (e.g.
        # TerrainLocomotion) determine which envs sit on which terrain type after the fact —
        # needed for gating tasks that require flat ground (e.g. a freely-simulated ball) to only
        # the envs that actually have it.
        self._terrain_type_grid: np.ndarray = np.full((self._num_rows, self._num_cols), "", dtype=object)
    ...
    def sample_env_origins(self) -> np.ndarray:
        ...
        # Cached so callers (e.g. TerrainLocomotion) can look up which terrain-type name each env
        # landed on via get_terrain_type_names(), using the exact same indices that produced these
        # origins — avoids a second, disagreeing RNG draw.
        self._last_terrain_levels = terrain_levels
        self._last_terrain_types = terrain_types
        return origin_grid[terrain_levels, terrain_types]

    def get_terrain_type_names(self, levels: np.ndarray, types: np.ndarray) -> np.ndarray:
        """Look up the terrain-type name (e.g. "flat", "rough") for each given (level, type) grid
        index pair. Returns an all-"" array for mesh types that never populate the per-cell grid
        (e.g. load_obj, none, or plain "plane" where the whole surface is uniformly flat anyway)."""
        grid = getattr(self, "_terrain_type_grid", None)
        if grid is None:
            return np.full(np.asarray(levels).shape, "", dtype=object)
        return grid[levels, types]
    ...
    def randomized_terrain(self) -> None:
        ...
        for k in range(self._num_sub_terrains):
            ...
            self._terrain_type_grid[i, j] = terrain_type
