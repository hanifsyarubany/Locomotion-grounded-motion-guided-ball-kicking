...
def create_video(
    video_frames, fps, save_dir, output_format="mp4", wandb_logging=True, episode_id=None, wandb_key="Training rollout"
):
    ...
    wandb_key : str, default="Training rollout"
        wandb media key to log the video under. Use distinct keys for simultaneously-active
        recorders (e.g. one per task) so each shows up as its own entry in the media panel.
    ...
    # Only non-empty for a non-default wandb_key (i.e. more than one recorder active
    # simultaneously, each with its own key — see BaseSimulator.add_video_recorder). Keeping it
    # empty for the default key leaves every existing single-recorder experiment's local filenames
    # byte-identical to before.
    label_prefix = ""
    if wandb_key != "Training rollout":
        label_prefix = re.sub(r"[^A-Za-z0-9_-]+", "_", wandb_key).strip("_") + "_"
    ...
    if wandb_logging and _is_wandb_available():
        ...
        # Local path: create permanent files. Two simultaneously-active recorders (see
        # add_video_recorder) can land on the same episode_id and the same integer-second
        # timestamp, so label_prefix disambiguates them; solo recorders (the common case) get no
        # prefix and keep their existing filenames.
        final_video = save_dir / f"{label_prefix}{episode_str}{timestamp}.mp4"
        temp_raw = save_dir / f"{label_prefix}{episode_str}temp_raw_{timestamp}.mp4"
        cleanup_files = False
