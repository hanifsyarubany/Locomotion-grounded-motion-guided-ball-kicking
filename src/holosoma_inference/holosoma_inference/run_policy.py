...
def _print_control_guide(policy_class, use_joystick: bool, dual_mode: bool = False):
    ...
    is_unified = policy_class.__name__ == "UnifiedPolicy"
    ...
    if use_joystick:
        ...
        if is_wbt:
            ...
            if is_unified:
                logger.info("")
                logger.info("⚠️  Kick trigger/return has no joystick binding yet — use keyboard mode.")
    else:
        ...
        if is_wbt:
            ...
            if is_unified:
                logger.info("")
                logger.info("Kick Controls:")
                logger.info("  k          - Trigger kick")
                logger.info("  l          - Return to locomotion control")
...
def main(annotated_config=None):
    ...
    # NOTE: --task.use-previous-version-policy is handled inside UnifiedPolicy.__init__ (and
    # therefore also NetworkControlledUnifiedPolicy/ApproachAndKickPolicy, both subclasses), not
    # here -- see that class's docstring for why centralizing it there instead of duplicating this
    # swap in every script's main() matters.
